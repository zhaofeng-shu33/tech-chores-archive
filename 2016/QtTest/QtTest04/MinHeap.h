...
#include <iostream>
using namespace std;
const int MINHEAP_H_DefaultSize = 100;												//默认的堆大小为1000
...
struct student{
    ...
    void printTeacher(){
        cout<<teacher[0]<<' '<<teacher[1]<<' '<<teacher[2]<<' '<<teacher[3]<<endl;
    }

   // student(int n,int x1=0,int y1=0):num(n),x(x1),y(y1){}
...
bool hasCommonOverTwo(student& a,student& b){
    int common=0;
    for(int i=0;i<4;i++)
        for(int j=0;j<4;j++)
            if(a.teacher[i]==b.teacher[j]&&b.teacher[j]!=0)
                common++;
